...
<script type="text/javascript">
                                ...
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        bootbox.alert({
                                            ...
                                            message: data,
                                            callback: function () {
                                                window.location.href = "showDeleteCustomer.action?pageNow=${page.pageNow }&name=${page.name}";
                                            }
                                        });
                                    }
                                });
	...
	function tiaozhuan(){
		window.location.href="showDeleteCustomer.action?pageNow="+$("#location").val()+"&name=${page.name}";
	}
</script>
</head>
...
<body>
	...
	<div class="place">
		...
		<ul class="placeul">
			<li><a href="#">首页</a></li>
			<li><a href="#">用户管理</a></li>
			<li><a href="#">查看已删除的用户</a></li>
		</ul>
    </div>
	<div class="rightinfo">
		<div class="tools">
			<ul style="float: left">
				<li style="display: inline-block;"><input type="button" class="btn  btn-warning" id="deleteCustomer" value="恢复用户"/></li>
			</ul>
		</div>
		...
		<div class="pagin">
			...
			<c:if test="${page.pageNow>1}">
					<li class="paginItem"><a href="showDeleteCustomer.action?pageNow=1&name=${page.name}">首页</a></li>
					<li class="paginItem"><a href="showDeleteCustomer.action?pageNow=${page.pageNow-1 }&name=${page.name}">上一页</a></li>
			</c:if>
			<c:if test="${page.pageNow==1}">
			...
			<c:if test="${page.pageNow<page.pageCount}">
				<li class="paginItem"><a href="showDeleteCustomer.action?pageNow=${page.pageNow+1 }&name=${page.name}">下一页</a></li>
				<li class="paginItem"><a href="showDeleteCustomer.action?pageNow=${page.pageCount }&name=${page.name}">尾页</a></li>
			</c:if>
			<c:if test="${page.pageNow==page.pageCount}">
